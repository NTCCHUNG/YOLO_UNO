...
Adafruit_MQTT_Subscribe nutnhanmaybom = Adafruit_MQTT_Subscribe(&mqtt, AIO_USERNAME "/feeds/nutnhan-maybom", MQTT_QOS_1);
Adafruit_MQTT_Subscribe nutnhanden = Adafruit_MQTT_Subscribe(&mqtt, AIO_USERNAME "/feeds/nutnhan-den", MQTT_QOS_1);
//Adafruit_MQTT_Subscribe onoffbutton = Adafruit_MQTT_Subscribe(&mqtt, OHS_USERNAME "/feeds/V1", MQTT_QOS_1);

// Setup a feed called 'photocell' for publishing.
Adafruit_MQTT_Publish cambiennhietdo = Adafruit_MQTT_Publish(&mqtt, AIO_USERNAME "/feeds/nhietdo");
Adafruit_MQTT_Publish cambiendoam = Adafruit_MQTT_Publish(&mqtt, AIO_USERNAME "/feeds/doam");
Adafruit_MQTT_Publish cambiendoamdat = Adafruit_MQTT_Publish(&mqtt, AIO_USERNAME "/feeds/doamdat");
Adafruit_MQTT_Publish cambienanhsang = Adafruit_MQTT_Publish(&mqtt, AIO_USERNAME "/feeds/anhsang");
...
void TaskLCDandMQTT(void *pvParameters);
...
void setup() {
  ...
  nutnhanmaybom.setCallback(onoffcallback);
  nutnhanden.setCallback(onoffcallback);
  //mqtt.subscribe(&slider);
  mqtt.subscribe(&nutnhanmaybom);
  mqtt.subscribe(&nutnhanden);
  ...
  xTaskCreate( TaskLightAndLED, "Task Light LED" ,2048  ,NULL  ,2 , NULL);
  xTaskCreate( TaskLCDandMQTT, "Task LCD and MQTT" ,2048  ,NULL  ,2 , NULL);
...
void TaskTemperatureHumidity(void *pvParameters) {  // This is a task.
  ...
  while(1) {                          
    ...
    dht20.read();
    nhietdo = dht20.getTemperature();
    doam = dht20.getHumidity();

    // Lam them chuc nang o day ...
...
void TaskLCDandMQTT(void *pvParameters) {
  ...
  while(1) {
    ...
    i++;
    if (i > 2) {
      if (cambiennhietdo.publish(nhietdo) 
      && cambiendoam.publish(doam)
      && cambienanhsang.publish(anhsang)
      && cambiendoamdat.publish(doamdat)) {
